...
// Initializes the Cicuit Span Value to 1
$('#circuitNumber').html(1);
$('#circuitNumber2').html(1);
...
console.log("hello there");
	...
	var pluginName = 'rotaryswitch',
		defaults = {
			minimum: 1, // Minimal value
			maximum: localStorage.getItem("activeCircuits"), // Maximum value
			step: 1, // Step size
	...
	Plugin.prototype = {
		...
		*/
		...
		renderMarks: function() {
			...
			for (; i < len; i += 1) {
				deg += degPerStep;
				var mark = $(this.htmlStructure.mark).css({'transform': 'rotate('+deg+'deg) translate(0, -'+ (this.domElements.main.width()/4.1 + (this.domElements.main.width()*0.1)) +'px)'});
				marks.append(mark);
			}
		...
		*/
		setValueToInput: function() {
			if (this.value !== this.lastTriggeredValue) {
				...
				//live updates the current cicuit value
				// starts at zero so add one
				$('#circuitNumber').html(this.value + 1);
				$('#circuitNumber2').html(this.value + 1);
				localStorage.setItem("currentCircuit", this.value );
...
$(document).ready(function () {
    $('#circuitNumber').html(circuit_value);
});
*/
